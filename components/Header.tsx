...
import { Theme, THEME_CATEGORIES } from '../types';
...
const Header: React.FC<HeaderProps> = ({ activeSection, theme, setTheme }) => {
  ...
  const ThemeSwitcherDropdown: React.FC = () => (
    <div className="absolute right-0 mt-2 w-56 bg-[var(--foreground)] rounded-md shadow-lg py-1 border border-[var(--card-border)] max-h-96 overflow-y-auto">
  ...
  return (
    <header className="fixed top-0 left-0 right-0 bg-[var(--foreground)]/80 backdrop-blur-sm z-40 shadow-sm transition-colors duration-300">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          <a href="#hero" onClick={(e) => handleNavClick(e, 'Home')} className="flex items-center space-x-3">
            <div className="w-10 h-10 rounded-full bg-gradient-to-r from-[var(--gradient-from)] to-[var(--gradient-to)] flex items-center justify-center text-white font-bold text-lg">
                MA
            </div>
            <span className="hidden sm:block font-semibold text-xl tracking-tight text-[var(--text-primary)]">{PORTFOLIO_DATA.name}</span>
          </a>
          ...
          <div className="hidden md:flex items-center space-x-4">
            <a 
                ...
                className="flex items-center px-4 py-2 bg-gradient-to-r from-[var(--gradient-from)] to-[var(--gradient-to)] text-white text-sm font-semibold rounded-md shadow-sm hover:opacity-90 transition-opacity"
            >
          ...
          <div className="flex items-center md:hidden">
            <div className="relative" ref={themeMenuRef}>
                 <button onClick={() => setIsThemeOpen(!isThemeOpen)} className="text-[var(--text-secondary)] hover:text-[var(--text-primary)] p-2 rounded-full focus:outline-none">
                    <ThemeIcon className="w-6 h-6" />
                 </button>
                 {isThemeOpen && <ThemeSwitcherDropdown />}
            </div>
            <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="text-[var(--text-primary)] hover:text-[var(--accent)] focus:outline-none ml-2">
      ...
      <div className={`absolute top-full left-0 right-0 bg-[var(--foreground)] shadow-lg md:hidden transition-all duration-300 ease-in-out ${isMenuOpen ? 'max-h-screen opacity-100 border-t border-[var(--card-border)]' : 'max-h-0 opacity-0'} overflow-hidden`}>
        <nav className="flex flex-col items-center space-y-2 p-4">
            {navLinks.map(link => (
             ...
             <a 
                href="/Maheshwar_Awale_Resume.pdf" 
                download="Maheshwar_Awale_Resume.pdf"
                className="w-full flex items-center justify-center mt-2 px-4 py-3 bg-gradient-to-r from-[var(--gradient-from)] to-[var(--gradient-to)] text-white text-sm font-semibold rounded-md shadow-sm hover:opacity-90 transition-opacity"
            >
                <DownloadIcon className="w-4 h-4 mr-2"/>
                Download Resume
            </a>
